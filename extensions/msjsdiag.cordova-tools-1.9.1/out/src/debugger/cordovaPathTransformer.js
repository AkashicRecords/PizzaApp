...
class CordovaPathTransformer extends vscode_chrome_debug_core_1.BasePathTransformer {
    ...
    setBreakpoints(source) {
        ...
        if (vscode_chrome_debug_core_1.utils.isURL(source.path)) {
            // already a url, use as-is
            vscode_chrome_debug_core_1.logger.log(`Paths.setBP: ${source.path} is already a URL`);
            return source;
        }
        const path = vscode_chrome_debug_core_1.utils.canonicalizeUrl(source.path);
        const url = this.getTargetPathFromClientPath(path);
        if (url) {
            source.path = url;
            vscode_chrome_debug_core_1.logger.log(`Paths.setBP: Resolved ${path} to ${source.path}`);
            return source;
        }
        else {
